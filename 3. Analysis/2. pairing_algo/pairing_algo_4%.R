#******************************************************************************
## Description .......: Pairs all M/W pair per race and year and calculate speed differences Method 2 / 2bis
##
...
##                      
## Outputs ...........: 'x%_pair_results.csv'
##                      'x%_pair_unique_results.csv'
##                    
#******************************************************************************
...
ans <- lapply(split(setDT(df), by = "catUTMB", sorted = TRUE), function(x) lapply(split(setDT(x), by = "annee"), 
...
for(i in seq(1,5)){
  ...
  print('Time to create pair for ', short, ' category')
  ...
  for(cat in seq(i+1,6)){ 
    ...
    print('Time to match paris from : ', paste(short, "vs", long))
    print((proc.time() - ptm) /60)
  }
  
}

# fwrite(result_tot, 'pairing_algo/4%_pair_results.csv', row.names = FALSE)
# fwrite(result_tot, 'pairing_algo/4%_pair_unique_results.csv', row.names = FALSE)
